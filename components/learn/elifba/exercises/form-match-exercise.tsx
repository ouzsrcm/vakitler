...
function pickRandomForm(letter: ILetter): string {
  const f = letter.forms;
  if (!letter.canConnect) {
    const pool = [f.isolated, f.final].filter(x => x && x !== "-");
    return pool[Math.floor(Math.random() * pool.length)] ?? f.isolated;
  }
  const pool = [f.isolated, f.initial, f.medial, f.final].filter(
    x => x && x !== "-"
  );
  return pool[Math.floor(Math.random() * pool.length)] ?? f.isolated;
}
...
export default function FormMatchExercise({
  deck,
  onExit,
  onFinish,
}: {
  ...
  const advance = () => {
    ...
    const nextLetter = deck[index + 1];
    setFormShown(nextLetter ? pickRandomForm(nextLetter) : "");
    setIndex(i => i + 1);
    setSelected(null);
    setDisabled(false);
    setFeedback("idle");
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <p className="text-center text-sm font-semibold text-zinc-800 dark:text-zinc-100">
            {t("elifba.exFormPrompt")}
          </p>
          <div
            className={cx(
              "rounded-2xl border p-10 text-center transition-colors duration-300",
              feedback === "correct" &&
          ...
          >
            <p
              className="font-arabic text-5xl font-bold text-zinc-900 dark:text-zinc-50"
              dir="rtl"
            >
              {formShown}
            </p>
